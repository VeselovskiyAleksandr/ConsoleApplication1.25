...
cout << "\n                Phonebook ";
         map<string, string> phonebook;
		 string subscr="", subscrib="", strTel="", strName="";
		 note subscriber;
		 fstream file("C:\\Users\\Александр\\Documents\\text for program\\phonebook.txt");
		 		if (file.is_open()) {
		 			cout << "\nThe file is open.\n";
		 	...
		 	while (!file.eof()) {
		      ...
		      cin >> subscr;
		 			if(phonebook.count(subscr)) {
		 				cout << "\n Subscriber with the specified phone number: ";
		 				cout<< subscr <<" " << phonebook[subscr];
		 			}
					else {
						map<string, string>::iterator nameSabs = phonebook.find(subscr);
						int count = 0;
						for (map< string, string>::iterator nameSabs = phonebook.begin(); nameSabs != phonebook.end(); nameSabs++) {
							if (nameSabs->second == subscr) {
								cout << "\nsubscriber with the specified name: ";
								cout << "\n" << nameSabs->first << " " << nameSabs->second;
								count++;
							}
						}
						if (count == 0) {
								cin>>subscrib;
								pair<string, string>addSubscriber(subscr, subscrib);
											phonebook.insert(addSubscriber);
											cout << "\nThe subscriber ";
											cout << addSubscriber.first << " " << addSubscriber.second;
											cout << " is added to the phone book.";
										subscriber.phoneNumber = addSubscriber.first;
											subscriber.family = addSubscriber.second;
											subscriber.listSubscribers.push_back(subscriber);
										}
